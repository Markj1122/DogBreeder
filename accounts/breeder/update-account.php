...
if (isset($_POST['new_email']) && isset($_FILES['profile_image'])) {
    ...
    if ($_FILES['profile_image']['size'] > 0) {
        // Upload and handle the new profile image for the breeders
        ...
        if (!in_array($profile_imageFileType, $profile_allowed_extensions)) {
            ...
            // Use a prepared statement to update breeders data in the database,
            // including the new email and the file path to the uploaded image.
            $currentDateTime = date('Y-m-d H:i:s');
            $stmt = $conn->prepare("UPDATE `user` SET user_email = ?, user_profile_image = ? WHERE user_id = ?");
            $stmt->bind_param("sss", $new_email, $profile_target_file,$_SESSION['user_id']);
